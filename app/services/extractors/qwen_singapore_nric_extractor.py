"""
Qwen3-VL Direct Singapore NRIC/FIN Extractor

Extracts Singapore NRIC/FIN Card fields directly from image using Qwen3-VL vision model.
...
class QwenSingaporeNricExtractor:
    """
    Direct Singapore NRIC/FIN extraction using Qwen3-VL vision model.

    Replaces the GLiNERIDCardExtractor (DocTR OCR + GLiNER NER + spatial coordinate extraction)
    with a single vision LLM call that extracts field values directly.

    Flow:
    1. Qwen3-VL extracts all NRIC/FIN fields from image
    2. Post-processing: Add confidence scores (1.0) for backwards compatibility
    3. Validate NRIC/FIN number format (S/T/F/G + 7 digits + check letter)
    4. Determine card type from prefix (S/T = NRIC, F/G = FIN)
    5. Normalize sex to single letter (M/F)
    6. Normalize DOB to ISO format
    7. Return structured data with confidence scores

    JSON Output Format (from LLM):
    {
        "nric_fin_number": "S1234567A",
    ...
    {
        "nric_fin_number": {"value": "S1234567A", "confidence": 1.0, "source": "vision_llm"},
        "full_name": {"value": "TAN AH KOW", "confidence": 1.0, "source": "vision_llm"},
        ...
    }
    """
    ...
    # NRIC/FIN prefix meanings
    NRIC_PREFIXES = ['S', 'T']  # Singapore Citizen / Permanent Resident
    FIN_PREFIXES = ['F', 'G']  # Foreigner
    ...
    async def extract_fields(
        ...
        """
        logger.info("Starting Qwen3-VL direct Singapore NRIC/FIN extraction")
        ...
        # Post-processing: Validate NRIC/FIN number format and determine card type
        card_type = self._validate_and_determine_card_type(extracted_data)
        if card_type:
            extracted_data["card_type"] = {
                "value": card_type,
                "confidence": 1.0,
                "source": "vision_llm"
            }
        ...
        logger.info(
            f"Qwen3-VL Singapore NRIC/FIN extraction completed: "
    ...
    def _build_system_prompt(self) -> str:
        """
        Build system prompt for Qwen3-VL direct Singapore NRIC/FIN extraction.

        CRITICAL: Must enforce JSON-only output to avoid reasoning mode.
        """
        return """Extract Singapore NRIC/FIN Card information and return ONLY JSON.
...
CRITICAL RULE FOR nric_fin_number:
- NRIC/FIN number format: 1 letter + 7 digits + 1 check letter
- Example: "S1234567A" or "F1234567N"
- First letter indicates card type:
  - S or T = NRIC (Singapore Citizen or Permanent Resident)
  - F or G = FIN (Foreigner Identification Number)
- Extract the EXACT number as shown on the card
- Remove spaces and dashes

CRITICAL RULE FOR full_name:
- Extract the cardholder's full name
- Singapore names are typically in uppercase
- Remove ALL salutations, titles, and prefixes
- Examples: "Mr. TAN AH KOW" -> "TAN AH KOW"

CRITICAL RULE FOR sex:
- Extract sex/gender if present
- Return as single letter: "M" or "F"
- Common indicators: "Male" -> "M", "Female" -> "F"

CRITICAL: Singapore NRIC/FIN cards do NOT have expiry dates.
- Do not extract any expiry or valid_until field
- These cards are valid for life

Remove ALL salutations, titles, and prefixes from names.
Examples: "Mr. JOHN DOE" -> "JOHN DOE"
  ...
  - MMM D, YYYY (e.g., "Jan 15, 1990")"""
    ...
    def _build_user_prompt(self) -> str:
        ...
        prompt_parts = [
            ...
            "Extract fields from this Singapore NRIC/FIN Card image.",
            "Return JSON with field values. Use null for missing fields.",
            "NRIC/FIN format: 1 letter + 7 digits + 1 check letter (e.g., S1234567A)"
        ]
    ...
    def _validate_and_determine_card_type(self, extracted_data: Dict[str, Any]) -> Optional[str]:
        """
        Validate NRIC/FIN number format and determine card type.

        Expected format: S/T/F/G + 7 digits + check letter
        S/T = NRIC (Singapore Citizen/PR)
        F/G = FIN (Foreigner)
        ...
        Returns:
            Card type: "NRIC" or "FIN", or None if cannot determine
        """
        if "nric_fin_number" in extracted_data:
            value = extracted_data["nric_fin_number"]["value"]
            if value and isinstance(value, str):
                # Clean up: remove spaces and make uppercase
                cleaned = re.sub(r'[\s\-]', '', value).upper()
                extracted_data["nric_fin_number"]["value"] = cleaned

                # Validate pattern and determine card type
                pattern = r'^([STFG])\d{7}[A-Z]$'
                match = re.match(pattern, cleaned)
                if match:
                    prefix = match.group(1)
                    card_type = "NRIC" if prefix in self.NRIC_PREFIXES else "FIN"
                    logger.info(f"Validated NRIC/FIN number: {cleaned}, card_type: {card_type}")
                    return card_type
                else:
                    logger.warning(f"NRIC/FIN number {cleaned} does not match expected format")
...
def get_qwen_singapore_nric_extractor() -> QwenSingaporeNricExtractor:
    """Get the singleton Qwen3-VL Singapore NRIC/FIN extractor instance."""
    global _instance
    if _instance is None:
        _instance = QwenSingaporeNricExtractor()
    return _instance
