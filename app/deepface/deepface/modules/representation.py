...
# LightPHE removed - not needed for our use case
...
def represent(
    ...
    cryptosystem: Optional[Any] = None,  # LightPHE removed
    ...
    batch_is_real, batch_antispoof_scores = [], []  # Anti-spoofing data
    ...
    for idx, single_img_path in enumerate(images):
        ...
        for img_obj in img_objs:
            # MODIFIED: Don't raise exception on spoof - return the score instead
            ...
            # Capture anti-spoofing data (if available)
            ...
            batch_is_real.append(is_real)
            batch_antispoof_scores.append(antispoof_score)
    ...
    for idy, batch_index in enumerate(batch_indexes):
        resp_obj = {
            ...
            # Anti-spoofing results (always included, defaults if not available)
            "is_real": batch_is_real[idy],
            "antispoof_score": batch_antispoof_scores[idy],
        }
