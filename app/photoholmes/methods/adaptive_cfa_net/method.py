...
class AdaptiveCFANet(BaseTorchMethod):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Since AdaptiveCFANet is itself a PyTorch model, we don't need to set self.model
        # The BaseTorchMethod will handle self-referencing models correctly
    ...
    def preprocess(self, image: Union[str, np.ndarray, Image.Image]) -> torch.Tensor:
        """
        Preprocess the input image for Adaptive CFA Net.
    ...
    def load_weights(self, weights: Union[str, Path, dict]) -> None:
        ...
        Args:
            weights: Path to weights file or dictionary of weights
        """
        if isinstance(weights, (str, Path)):
            weights_path = str(weights)
            if not os.path.exists(weights_path):
                logger.warning(f"Weights file not found: {weights_path}")
                logger.info("Initializing with random weights instead")
                self.init_weights()
                return
            ...
            try:
                ...
                logger.info("=> weights loaded successfully from dictionary")
            except Exception as e:
                logger.warning(f"Error loading weights from dictionary: {e}")
                self.init_weights()
    ...
    def predict(self, image: Union[str, np.ndarray, Image.Image]) -> Dict[str, Any]:
        ...
        if isinstance(image, str):
            ...
            image_size = (image.shape[1], image.shape[0]) if hasattr(image, 'shape') else (image.shape[2], image.shape[1])
        ...
        return {
            'heatmap': pred,
            'upscaled_heatmap': upscaled_heatmap,
            'best_grid': best_grid,
