#!/usr/bin/env python
"""
Apply the 011_add_myanmar_banks.sql migration.
"""
import re
...
def apply_migration():
    ...
    with open('schema/migrations/011_add_myanmar_banks.sql', 'r') as f:
        sql = f.read()
    ...
    try:
        ...
        # Verification
        cursor.execute("SELECT COUNT(*) FROM banks WHERE country_code='MM'")
        myanmar_banks = cursor.fetchone()[0]
        print(f"Myanmar banks in DB: {myanmar_banks}")

        cursor.execute("SELECT swift_code, legal_name FROM banks WHERE country_code='MM'")
        print("Myanmar banks:")
